...
const EditLodgingDetailData = ({ onSelect }) => {
  const { no } = useParams();
  ...
  useEffect(() => {
    fetch(`http://${API_SERVER}/api/manager/packages/lodgingDetail/${no}`, {
      ...
      .then((data) => {
        //data.a , data.b 이런식으로 키밸류를 분류 a-b는 서비서에서 지정한 이름으로 뽑아오면 된다(from 재성)
        console.log('API 응답 데이터:', data);
        // setLodging(Array.isArray(data.lodging) ? data.lodging : [data.lodging]);
        setLodging(data.lodging);
        setlodgingAttchment(data.lodgingAttchment);
        console.log(data.lodgingAttchment);
        setLoading(false);
      })
      .catch((error) => {
      ...
      <NaviButtons>
        ...
        <PrevStep
          onClick={() => {
            ...
            navigate('/manager/edit/package/lodging/list'); // 이전 페이지 이동
          }}
        >
          뒤로
        </PrevStep>
...
export default EditLodgingDetailData;
