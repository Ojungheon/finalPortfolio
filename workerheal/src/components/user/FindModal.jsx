...
const FindModal = ({ mode, onClose }) => {
  ...
  const handleFindId = async (event) => {
    ...
    try {
      const response = await fetch(`http://${API_SERVER}/api/member/findId`, {
        method: 'POST',
  ...
  const handleFindPw = async (event) => {
    ...
    try {
      const response = await fetch(`http://${API_SERVER}/api/member/findPw`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          id: formData.id,
      ...
      if (response.ok) {
        ...
        dispatch(setUserId(formData.id)); // ✅ Redux로 ID 저장
  ...
  return (
    <ModalOverlay>
      <ModalContent>
        ...
        <h3>{mode === 'findId' ? '아이디 찾기' : '비밀번호 찾기'}</h3>
