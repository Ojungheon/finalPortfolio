...
import UpdateImgFiles from '../../../common/components/UpdateImgFiles.jsx';
...
const TourEdit = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (imgList.length > 0) {
      ...
      // 신규 등록 이미지 없이 기존 이미지 모두 지운 경우 예외처리
      if (beforeList.length == deleteList.length) {
        Toast2.fire({
          icon: 'error',
    ...
    // 삭제 된 이미지 리스트 추가
    fd.append('deleteNoList', deleteList);
  ...
  const getTourInfo = async () => {
    ...
    try {
      const data = await getTourDetail(no);

      const tempData = {
        no: data.tourVo.no,
        managerNo: data.tourVo.managerNo,
        regionNo: data.tourVo.regionNo,
        categoryNo: data.tourVo.categoryNo,
        name: data.tourVo.name,
        phone: data.tourVo.phone,
        price: data.tourVo.price,
        detail: data.tourVo.detail,
        linkPath: data.tourVo.linkPath,
        tag:
          data.tourVo.tag == null
            ? []
            : data.tourVo.tag.includes(',')
            ? data.tourVo.tag.split(',')
            : [data.tourVo.tag],
        facilitieCode:
          data.tourVo.facilitieCode == null
            ? []
            : data.tourVo.facilitieCode.includes(',')
            ? data.tourVo.facilitieCode.split(',')
            : [data.tourVo.facilitieCode],
        postcode: data.tourVo.postcode,
        roadAddress: data.tourVo.roadAddress,
        detailAddress: data.tourVo.detailAddress,
      };

      setFacilityList(data.tourVo.facilitieCode.split(','));
      setFormData(tempData);
      setBeforeList(data.imgList);
    } catch (error) {
      await BlockAlert({
        icon: 'error',
        title: error.message,
          ...
          <div>
            <LineWrap>
              ...
              <RegionSelect
                ...
                size={''}
                color={'primary'}
                value={formData.regionNo ? formData.regionNo : ''}
                handler={handleInputChange}
              ...
              <TourCategorySelect
                ...
                size={''}
                color={'primary'}
                value={formData.categoryNo ? formData.categoryNo : ''}
                handler={handleInputChange}
                labelSx={{ fontSize: 16, mt: 2, ml: 3 }}
            ...
            <SpaceBetween>
              ...
              <MuiTextField
                ...
                variant={'filled'}
                color={'primary'}
                value={formData.linkPath ? formData.linkPath : ''}
                isFocused={true}
