...
const InsertRoomtype = () => {
  const { lodgingNo } = useParams();

  const navigate = useNavigate();

  // lodgingNo 값 확인용 useEffect
  useEffect(() => {
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await fetch(
        `http://${API_SERVER}/api/host/tempRoomtype/insert`,
        {
          method: 'POST',
          headers: {
            token: `${token}`,
      ...
      Swal.fire({
        ...
        navigate(`/host/tempRoomtype/list/${lodgingNo}`);
      });
    } catch (error) {
      Swal.fire({
        title: '등록 오류',
    ...
    <>
      ...
      <Layout>
        ...
        <TagAndFacilityWrapper>
          <RoomtypeFacility formData={formData} updateField={updateField} />
        </TagAndFacilityWrapper>
        <DetailsWrapper>
